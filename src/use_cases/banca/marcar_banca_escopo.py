...
class MarcarBancaEscopoUseCase:
    ...
    def _janela_do_escopo(self, escopo):
        # ⭐ O calendário base DESTE escopo (`projeto_escopo.calendario` dentro
        # da frente dele): a janela em que a banca precisa caber (§9) é contada
        # no calendário do curso do time, não no da frente vizinha.
        dias_nao_letivos = [
...
class RegistrarRealizacaoBancaUseCase:
    ...
    def __init__(self, db: Session):
        ...
        # volta uma regra que o núcleo decidiu derrubar, e ainda por cima sem o
        # import, que saiu junto.
        self.sessao_repository = BancaSessaoRepository(db)
